...
{
    //模型2老版本
    public class GeneralSchedule_Model_2_Old
    {      
        private static GeneralSchedule_Model_2_Old instance;
        private static TimeSpan m_tTaskEstimate = new TimeSpan(0,0,5,0);
        private static TimeSpan m_tBalanceTime = new TimeSpan(0,0,9,0);

        public static  GeneralSchedule_Model_2_Old Instance
        {
            get
            {
                if (instance == null)
                {
                    instance = new GeneralSchedule_Model_2_Old();
                }
                return instance;
            }
        }

        private GeneralSchedule_Model_2_Old()
        {
        }
        ...
        /// <returns></returns>
        public bool Schedule_Mode2(DBData_TOS dbData_Schedule,
                                      ref OrderedDecisionTable jobsOrderedDecisionTable)
        {
            ...
            DateTime tEarliest = DateTime.MinValue;
            DateTime tLatest   =  DateTime.MinValue;
            if (!WQRelateWI(ScheduleDBData, ref tEarliest, ref tLatest, ref WQWIList))
                return false;
            
            //if (!CutWIListByTime(tEarliest + new TimeSpan(0, 0, 30, 0), ref  WQWIList))
            //    return false;
            ...
            for (int i = 0; i < WIList.Count; i++)
            {
                for (int j = i; j < WIList.Count; j++)
                {
                    if (j > i)
                    {
                        ...
                        {
                            if (WIList[j].T_StartTime > tFirstEnd && tFirstEnd > DateTime.MinValue)
                            {
                                matrix[i, j] = -1;
                                matrix[j, i] =  1;
                            }
