...
namespace ZECS.Schedule.DBDefine.NewBlockInfo
{
    ...
    public class RMG
    {
        ...
        public bool Enable;
        public TE_ARMGDevice tEstimate = new TE_ARMGDevice();
    }
    ...
    public class BlockInfo
    {
       ...
       // EstimateTime
        private BmsPosition ParseAscLocation(String location)
        {
            if (location == null)
                return null;
            
            location = location.Trim();
            String[] aryStr = location.Split('.');
            if(aryStr.Count() < 3)
                return null;

            BmsPosition bmsPos = new BmsPosition();
            try
            {
                bmsPos.BlockName = this.BlockName;
                bmsPos.BayType = ToBayType(aryStr[0]);
                if (bmsPos.BayType == BmsBayType.None)
                    return null;
                bmsPos.Bay = int.Parse(aryStr[1]);
                bmsPos.Lane = int.Parse(aryStr[2]);
                bmsPos.Tier = 6;
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public bool GetBMSPositionEx(BmsPosition pos, out BmsPositionEx posEx)
        {
            posEx = null;

            Bay bay = null;
            if (pos.BayType == BmsBayType.Block)
            {
                bay = YardArea.BayList.Find(obj => obj.BayType == pos.BayType
                       && obj.BayNo == pos.Bay);
            }
            else if (pos.BayType == BmsBayType.WS)
            {
                bay = WSExchangeArea.BayList.Find(obj => obj.BayType == pos.BayType
                         && obj.BayNo == pos.Bay);
            }
            else if (pos.BayType == BmsBayType.LS)
            {
                bay = LSExchangeArea.BayList.Find(obj => obj.BayType == pos.BayType
                       && obj.BayNo == pos.Bay);
            }
            else
                return false;
        ...
        public bool GetYardPositionEx( BlockCoordinate coordinate, BmsBaySize baySize , out BmsPositionEx posEx)
        {
            posEx = null;
        ...
        /// <returns></returns>
        public Bay GetAvoidYardBay(int xTo, bool bPositiveDirect)
        {
            if (bPositiveDirect)
            {
    ...
    public class BMSInfo
    {
        ...
        {
            BMSInfoFactory factory =  BMSInfoFactory.Instance;
            List<BlockInfo> blockInfoList = factory.BlockInfoList;
            IsInitialize = false;
            if (blockInfoList.Count < 1)
                blockInfoList = factory.GetDefaultBlockInfoList();
    ...
    {
        private static BMSInfoFactory m_Instance;
        public static BMSInfoFactory Instance
        {
            get
            {
                if (m_Instance == null)
                {
                    ...
                    if (m_Instance == null)
                        m_Instance = new BMSInfoFactory();
                }

                return m_Instance;
            }
        }
        private  static String m_DefaultPath = "BMSInfo.xml";

        public  List<BlockInfo> BlockInfoList = new List<BlockInfo>();

        private BMSInfoFactory()
        {
        }

        public  List<BlockInfo> GetDefaultBlockInfoList()
        {
            BlockInfoList.Clear();
        ...
        private TE_ARMGDevice CreateTimeEstimateObject(String strID)
        {
            TE_ARMGDevice tEstimate = new TE_ARMGDevice();

            tEstimate.ID = strID;
            tEstimate.Gantry.Trolley.Hoister.NoloadSpeed = 3;
            tEstimate.Gantry.Trolley.Hoister.LoadedSpeed = 0.4;
            tEstimate.Gantry.Trolley.Hoister.MinAccTime = 3;
            tEstimate.Gantry.Trolley.Hoister.MinAccTime = 3;
            tEstimate.Gantry.Trolley.Hoister.SafePosition = 18.1;
            tEstimate.Gantry.Trolley.Hoister.Position = 18.1;
            tEstimate.Gantry.Trolley.NoloadSpeed = 1.2;
            tEstimate.Gantry.Trolley.LoadedSpeed = 1.2;
            tEstimate.Gantry.Trolley.MinAccTime = 3;
            tEstimate.Gantry.Trolley.MinDecTime = 3;
            tEstimate.Gantry.Trolley.Position = 0;
            tEstimate.Gantry.NoloadSpeed = 4;
            tEstimate.Gantry.LoadedSpeed = 4;
            tEstimate.Gantry.MinAccTime = 3;
            tEstimate.Gantry.MinDecTime = 3;
            tEstimate.Gantry.Position = 100;
        ...
        private BlockInfo CreateDefaultBlockInfo(int blockNo)
        {
            ...
            LSRmg.Enable = true;
            LSRmg.JobTypeList = new List<JobType>() { JobType.SBLOCK, JobType.RECEIVE, JobType.DELIVERY };
            LSRmg.tEstimate = CreateTimeEstimateObject(LSRmg.ID);
            ...
            WSRmg.tEstimate = CreateTimeEstimateObject(WSRmg.ID);
        ...
        /// </summary>
        /// <returns></returns>
        private static BMSInfoFactory LoadConfig()
        {
            return LoadConfig(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, m_DefaultPath));
        }
        ...
        /// <returns></returns>
        private static BMSInfoFactory LoadConfig(string path)
        {
        ...
        /// <returns></returns>
        private bool SaveConfig(BMSInfoFactory info)
        {
        ...
        /// <param name="path"></param>
        /// <returns></returns>
        private bool SaveConfig(BMSInfoFactory info, string path)
        {
            try
            {
